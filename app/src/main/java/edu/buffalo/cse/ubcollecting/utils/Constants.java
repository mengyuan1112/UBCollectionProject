package edu.buffalo.cse.ubcollecting.utils;

public class Constants {

    //Roles
    public static final String ROLE_ADMIN = "Admin";
    public static final String ROLE_INTERVIEWER = "Interviewer";
    public static final String ROLE_CONSULTANT = "Consultant";

    //Questionnaire Type
    public static final String REGULAR = "Regular";
    public  static final String INTRODUCTORY = "Introductory";
    public static final String SOCIOLINGUISTIC = "Sociolinguistic";
    public static final String EMERGENCY = "Emergency";
    public static final String TEST = "Test";

    //Question Property Definition
    public static final String AUDIO = "Audio";
    public static final String VIDEO = "Video";
    public static final String PHOTO = "Photo";
    public static final String TEXT = "Text";
    public static final String LIST = "List";
    public static final String LOOP = "Loop";
    public static final String MULTICHOICE = "Multi_Choice";

    //Language Type
    public static final String LWC = "LWC";
    public static final String RESEARCH_LANG = "Research Language";
    public static final String REGIONAL = "Regional";

    //Language Name
    public static final String ENGLISH = "English";
    public static final String FRENCH = "French";
}
